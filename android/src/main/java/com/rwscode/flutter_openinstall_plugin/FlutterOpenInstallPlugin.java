package com.rwscode.flutter_openinstall_plugin;
...
public class FlutterOpenInstallPlugin implements FlutterPlugin, MethodCallHandler, ActivityAware, PluginRegistry.NewIntentListener {
    ...
    private static final String METHOD_OP_ID = "getOpid";
    ...
    public void onAttachedToEngine(@NonNull FlutterPluginBinding binding) {
        ...
        channel = new MethodChannel(flutterPluginBinding.getBinaryMessenger(), "flutter_openinstall_plugin");
...
//                result.error("ERROR", "pointId is empty or pointValue is null", null);
            ...
            if (TextUtils.isEmpty(shareCode) || TextUtils.isEmpty(sharePlatform)) {
                ...
                OpenInstall.reportShare(shareCode, sharePlatform, (v, error) -> {
                    boolean shouldRetry = error != null && error.shouldRetry();
                    data.put("shouldRetry", shouldRetry);
                    if (error != null) {
                        data.put("message", error.getErrorMsg());
        ...
        } else if (METHOD_OP_ID.equalsIgnoreCase(call.method)) {
...
//                configuration.getImei(), configuration.getMacAddress()));
    ...
    private final PluginRegistry.RequestPermissionsResultListener permissionsResultListener = (requestCode, permissions, grantResults) -> {
